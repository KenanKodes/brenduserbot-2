...
async def _(dyno):
    if app is None:
        return await dyno.edit("**Heroku Xətası** `HEROKU_APPNAME` **və** `HEROKU_APIKEY` **doldurun**")
    await dyno.edit("**Heroku Hesabatı alınır**")
    with open("BrendLog.txt", "w") as log:
        log.write(app.get_log())
    await dyno.client.send_file(
        entity=dyno.chat_id,
        file="BrendLog.txt",
        thumb="userbot/resources/Brend_Logo.jpg",
        caption=f"**{DEFAULT_NAME} - Heroku Logudur**",
    )
    await dyno.delete()
    return os.remove("BrendLog.txt")
