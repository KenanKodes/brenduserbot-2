...
def butonla(sayfa, moduller):
    ...
    butonlar.append([custom.Button.inline("👈🏻 Geri", data=f"sayfa({(max_pages - 1) if sayfa == 0 else (sayfa - 1)})"), custom.Button.inline("❎ Close ❎", data="close"), custom.Button.inline("İləri 👉🏻", data=f"sayfa({0 if sayfa == (max_pages - 1) else sayfa + 1})")])
...
with bot:
    ...
    try:
        ...
        async def sayfa(event):
            ...
            veriler = butonlastir(sehife, CMD_HELP)
        ...
        async def ofen(event):
            if not event.query.user_id == uid: 
                return await event.answer("❌ Hey! Mənim mesajlarıma toxunma! Öz hesabına @BrendUserbot qur.", cache_time=0, alert=True)
            sayfa = int(looters)
            veriler = butonla(sayfa, CMD_HELP)
            text = f"**⚡ 𝐁𝐫𝐞𝐧𝐝 𝐔𝐬𝐞𝐫𝐛𝐨𝐭​**\n\n**📥 Yüklənən modul sayı:** `{len(CMD_HELP)}`\n**📄 Səhifə:** {sayfa + 1}/{veriler[0]}"
            await event.edit(text, file=helplogo,  buttons=veriler[1],  link_preview=False)
        ...
        async def sayfa(event):
            ...
            await event.edit("Bağla", buttons=buttons)
        ...
        async def bilgi(event):
            ...
            try:
                butonlar = [custom.Button.inline("💎 " + cmd[0], data=f"komut[{emr}[{sehife}]]({cmd[0]})") for cmd in CMD_HELP_BOT[emr]['commands'].items()]
            except KeyError:
                return await event.answer("❌ Bu modula açıqlama yazılmayıb.", cache_time=0, alert=True)
            butonlar = [butonlar[i:i + 2] for i in range(0, len(butonlar), 2)]
            butonlar.append([custom.Button.inline("👈🏻 Geri", data=f"sayfa({sayfa})")])
            await event.edit(f"**📂 Fayl:** `{komut}`\n**🔢 Əmr sayı:** `{len(CMD_HELP_BOT[komut]['commands'])}`", file=helplogo, buttons=butonlar, link_preview=False)
        ...
        async def komut(event):
            ...
            await event.edit(result, file=helplogo, buttons=[custom.Button.inline("👈🏻 Geri", data=f"bilgi[{sayfa}]({cmd})")], link_preview=False)
